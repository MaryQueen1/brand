...
addButtonEls.forEach((addButtonEl) => {
  addButtonEl.addEventListener("click", (e) => {
    // e.preventDefault();
    const productToAdd = {
      title: addButtonEl.parentElement.parentElement.querySelector(
        ".products__item__name"
      ).textContent,
      imgPath: addButtonEl.parentElement.parentElement.querySelector(
        ".products__item__image"
      ).src,
      price: Number(
        addButtonEl.parentElement.parentElement
...
for (let i = 0; i < localStorage.length; i++) {
  productsinCart.insertAdjacentHTML(
    "beforeend",
    `
        <div class="sh__cart__table__productLine">
            <div class="sh__cart__table__productDetails">
                <a class="sh__cart__table__productImg" href="single_page.html">
                    <img class="sh__cart__table__productImgPath" src=${
                      JSON.parse(localStorage.getItem(localStorage.key(i)))
                        .imgPath
                    } alt="">
                </a>
                    <div class="sh__cart__table__productText">
                        <h3 class="sh__cart__table__productName">${
                          JSON.parse(localStorage.getItem(localStorage.key(i)))
                            .title
                        }</h3>
                        ...
                        <span class="sh__cart__table__productPrice">$${
                          JSON.parse(localStorage.getItem(localStorage.key(i)))
                            .price
                        }</span>
                        ...
                        <span class="sh__cart__table__productSubtotal">$${
                          JSON.parse(localStorage.getItem(localStorage.key(i)))
                            .price
                        }</span>
...
for (let i = 0; i < localStorage.length; i++) {
  productsinCartMini.insertAdjacentHTML(
    "afterbegin",
    `
<a class="cartBox__shiftToItem" href="single_page.html">
<div class="cartBox__product">
    <div class="cartBox__productImg"><img class="cartBox__productImage"src=${
      JSON.parse(localStorage.getItem(localStorage.key(i))).imgPath
    } alt=""></div>
    <div class="cartBox__productDescription">
        <h3 class="cartBox__itemName">${
...
for (let i = 0; i < localStorage.length; i++) {
  ...
  totalAmountValue += JSON.parse(
    localStorage.getItem(localStorage.key(i))
  ).price;
}
const grandTotalVal = document.querySelector(
...
const removeIconEls = document.querySelectorAll(
  ".sh__cart__table__productAction"
);
removeIconEls.forEach((removeIcon) => {
  removeIcon.addEventListener("click", (e) => {
    const thisProductImgPath = e.currentTarget.parentElement.querySelector(
      ".sh__cart__table__productImgPath"
    ).src;
    for (let i = 0; i < localStorage.length; i++) {
      if (
        JSON.parse(localStorage.getItem(localStorage.key(i))).imgPath ===
